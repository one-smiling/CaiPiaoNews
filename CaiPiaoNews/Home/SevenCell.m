...
-(void)setModel:(CategoryModel *)model
{
    ...
    if ([model.cn isEqualToString:@"双色球"] ||[model.cn isEqualToString:@"七星彩"])
    {
        ...
        for (UILabel *L in _bigV.subviews)
        {
            ...
            if ([model.cn isEqualToString:@"双色球"] &&L.tag==6)
            {
   ...
   else
   {
       ...
       NSArray *dataA = [model.code componentsSeparatedByString:@","];
       [arrs addObjectsFromArray:dataA];
       [arrs removeObjectAtIndex:4];
       NSString *last = dataA[4];
       NSArray *dataB = [last componentsSeparatedByString:@"#"];
       [arrs insertObject:[dataB firstObject] atIndex:4];
       [arrs insertObject:[dataB lastObject] atIndex:5];
       for (UILabel *L in _bigV.subviews)
       {
           L.text = arrs[L.tag];
           if (L.tag == 6|| L.tag == 5) {
