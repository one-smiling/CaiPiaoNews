...
-(void)getData
{
    [NetworkManager GET:@"http://cai88.com/api/prizecenter.action" parameters:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        NSArray *arr = responseObject[@"model"];
        ...
        [self.tableView.mj_header endRefreshing];
        [self.view toastMessage:@"请检查网络是否正常"];
...
-(void)headUploadingisHead:(NSInteger)number
{
    ...
    if (number ==1)
    {
        ...
        [para setValue:_categorys forKey:@"type"];
        [para setValue:@(20) forKey:@"ps"];
        [para setValue:@"1" forKey:@"pn"];

        [NetworkManager GET:@"http://cai88.com/api/list.action/?" parameters:para success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            // NSLog(@"----------%@",data);
    ...
    }else
    {
        ...
        NSMutableDictionary *para = [[NSMutableDictionary alloc]init];
        [para setValue:_categorys forKey:@"type"];
        [para setValue:@(self.nextPage) forKey:@"ps"];
        [para setValue:@"1" forKey:@"pn"];
        [NetworkManager GET:@"http://cai88.com/api/list.action/?" parameters:para  success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            // NSLog(@"----------%@",data);
            NSDictionary *dict = responseObject[@"model"];
